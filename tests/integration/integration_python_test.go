...
func TestConfigGetterOverloads(t *testing.T) {
	...
	integration.InstallPythonDependencies(t, e.CWD)
		...
		t.Run(runtime, func(t *testing.T) {
			integration.ProgramTest(t, &integration.ProgramTestOptions{
				PrepareProject: func(info *engine.Projinfo) error {
					...
					if runtime == "python" {
						integration.InstallPythonDependencies(t, info.Root)
					}
...
func TestPythonComponentProviderInComponentProvider(t *testing.T) {
	...
	integration.ProgramTest(t, &integration.ProgramTestOptions{
		...
		PrepareProject: func(info *engine.Projinfo) error {
			...
			out, err := cmd.CombinedOutput()
			require.NoError(t, err, "`%s` in %s failed err: %s, out: %s", cmd.String(), cmd.Dir, out, err)
