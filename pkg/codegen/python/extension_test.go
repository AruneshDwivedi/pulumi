...
// schema generates a Python SDK whose get_package() fills the new
...
func TestExtensionParameterizationCodegen(t *testing.T) {
	...
	for path, content := range files {
		if strings.HasSuffix(path, "_utilities.py") {
			utilities = string(content)
		}
	}
	require.NotEmpty(t, utilities, "expected a _utilities.py in the generated SDK")
	assert.Contains(t, utilities, "extension=parameterization",
		"an extension SDK's get_package() should fill RegisterPackageRequest.extension")
	assert.NotContains(t, utilities, "parameterization=parameterization",
