...
func TestESCConfigEditorArrayIndexPathRejected(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	s := remoteStackForEditor(t, []byte("values:\n  pulumiConfig: {}\n"), "etag", nil)
	editor, err := newConfigEditor(ctx, s, &workspace.ProjectStack{}, config.NopEncrypter, "")
	require.NoError(t, err)
	key := config.MustMakeKey("testProject", "foo[0]")
	err = editor.Set(ctx, key, config.NewValue("x"), true)
	require.ErrorContains(t, err, "pulumi config edit")
}
