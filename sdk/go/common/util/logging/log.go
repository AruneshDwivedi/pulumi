...
var (
	...
	primaryWrapper func(slog.Handler) slog.Handler                    // optional wrapper for the primary handler
...
func rebuildLogger() {
	...
	// The filtering handler (secret redaction) wraps the multiHandler
	// so it applies to all outputs.  The sink and export handlers
	// receive the raw format string + individual argN attributes.
...
// SetPrimaryWrapper installs a function that wraps the primary
// handler.  This is used to transform attributes (e.g. decode
// property value bytes to readable strings) before they reach the
// primary log output.  Pass nil to remove the wrapper.
func SetPrimaryWrapper(w func(slog.Handler) slog.Handler) {
	handlerMu.Lock()
